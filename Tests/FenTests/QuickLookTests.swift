...
struct QuickLookTests {
    ...
    @Test @MainActor
    func controlCharacterContentRendersAsLiteralTextRatherThanCrashing() async throws {
        ...
        let html = try await QuickLookPreviewRenderer().render(fileURL: fixture.fileURL)
        #expect(html.contains("<html"))
    }

    @Test @MainActor
    func genuinelyUndecodableDataThrowsRatherThanCrashing() async throws {
        ...
        await #expect(throws: QuickLookPreviewRenderer.RenderError.unreadable) {
            try await QuickLookPreviewRenderer().render(fileURL: fixture.fileURL)
        }
    }
    ...
    @Test @MainActor
    func fileOverTheSizeCapThrowsRatherThanRendering() async throws {
        ...
        await #expect(throws: QuickLookPreviewRenderer.RenderError.fileTooLarge(byteCount: oversized.count)) {
            try await QuickLookPreviewRenderer().render(fileURL: fixture.fileURL)
        }
    }

    @Test @MainActor
    func renderingAOneMegabyteFileStaysUnderABoundedTime() async throws {
        ...
        _ = try await QuickLookPreviewRenderer().render(fileURL: fixture.fileURL)
    ...
    @Test @MainActor
    func mermaidScriptIsAbsentWhenPreferenceIsOff() async throws {
        ...
        let html = try await QuickLookPreviewRenderer().render(fileURL: fixture.fileURL, preferences: preferences)
