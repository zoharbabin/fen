...
open class Highlightr
{
    ...
    public init?(highlightPath: String? = nil)
    {
        ...
        #if SWIFT_PACKAGE
        // The SPM-generated Bundle.module accessor resolves against
        // Bundle.main.bundleURL (the .app root), but signed .apps must place
        // resource bundles inside Contents/Resources/ (Bundle.main.resourceURL).
        // Prefer resourceURL so the lookup works in distributed builds; fall back
        // to Bundle.module for local development builds where the build-path
        // fallback in the generated accessor handles the rest.
        let resourceBase = Bundle.main.resourceURL ?? Bundle.main.bundleURL
        let bundleURL = resourceBase.appendingPathComponent("Highlightr_Highlightr.bundle")
        let bundle = Bundle(url: bundleURL) ?? Bundle.module
        #else
