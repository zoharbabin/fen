...
public struct DocumentHTMLExporter: Sendable {
    ...
    /// folder its resolved assets belong in. `async` and `@MainActor` since raw HTML sanitization
    /// (issue #118) runs the vendored DOMPurify inside a hidden `WKWebView` (also `@MainActor`),
    /// the only DOM-capable JS engine available, which only exposes `evaluateJavaScript`
    /// asynchronously.
    @MainActor
    public func export(
        markdown: String,
